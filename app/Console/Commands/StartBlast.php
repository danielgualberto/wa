...
class StartBlast extends Command
{
    ...
    public function handle()
    {
        ...
        foreach ($waitingCampaigns as $campaign) {
            ...
            if ($isSenderConnected && $campaign->schedule <= now()/*date('Y-m-d h:i:sa')*/) {
                    ...
                    if ($blastcount > 0) {
                        ...
                        try {
                            $proc = Http::withOptions(['verify' => false])
                             ...
                             Log::info($proc);
                              Blast::whereIn('receiver', $successNumber)->where('campaign_id', $campaign->id)->whereStatus('pending')->update(['status' => 'success']);
                              Blast::whereIn('receiver', $failedNumber)->where('campaign_id', $campaign->id)->whereStatus('pending')->update(['status' => 'failed']);
