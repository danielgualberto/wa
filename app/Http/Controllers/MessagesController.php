...
class MessagesController extends Controller
{
    ...
    public function textMessageTest(Request $request){
        ...
        if($number->status == 'Disconnect'){
            return redirect()->back()->with('alert',['type' => 'danger','msg' => __('validation.sender_not_connected')]);
        }
        ...
        $number->save();
        return redirect()->back()->with('alert',['type' => 'success','msg' => __('validation.message_sent')/*. json_encode($sendMessage->data)*/]);
    }
    ...
    public function buttonMessageTest(Request $request){
        ...
        if(!$request->has('button')){
            return redirect()->back()->with('alert',['type' => 'danger','msg' => __('validation.no_buttons_selected')]);
        }
        $buttons = [
    ...
    public function templateMessageTest(Request $request){
      if(!$request->has('template')){
            return redirect()->back()->with('alert',['type' => 'danger','msg' => __('validation.no_template_selected')]);
        }
        ...
        $templates = [];
            ...
            try {
                ...
                foreach($request->template as $template){
                 ...
                 if (!in_array($type, $allowType)) {
                    return back()->with('alert', [
                        'type' => 'danger',
                        'msg' => __('validation.template_not_valid')
                    ]);
                 }
               ...
               return redirect()->back()->with('alert',['type' => 'danger','msg' => __('validation.template_not_valid')]);
            }
    ...
    public function listMessageTest(Request $request){
       if(!$request->has('list')){
            return redirect()->back()->with('alert',['type' => 'danger','msg' => __('validation.no_list_selected')]);
        }
