...
class AdminController extends Controller
{
    ...
    public function userStore(Request $request){
        ...
        if($request->active_subscription == 'active'){
            ...
            if($request->subscription_expired < date('Y-m-d')){
                return redirect()->back()->with('alert' , ['type' => 'danger', 'msg' => __('validation.subscription_expired_greater')]);
            }
        }
        ...
        $user->save();
        return redirect()->back()->with('alert', ['type' => 'success', 'msg' => __('validation.user_created')]);
    ...
    public function userUpdate(Request $request){
        ...
        $user->subscription_expired = $request->subscription_expired ?? null;
        $user->save();
        return redirect()->back()->with('alert', ['type' => 'success', 'msg' => __('validation.user_updated')]);
    }

    public function userDelete($id){
        $user = User::find($id);
        if($user->level == 'admin'){
            return redirect()->back()->with('alert', ['type' => 'danger', 'msg' => __('validation.user_delete_is_admin')]);
        }
        ...
        $user->delete();
        return redirect()->back()->with('alert', ['type' => 'success', 'msg' => __('validation.user_deleted')]);
    }
}
