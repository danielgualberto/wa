...
class BlastController extends Controller
{
    ...
    public function blastProccess(Request $request)
    {
        ...
        //             'type' => 'danger',
        //             'msg' =>
        //                 __('validation.campaign_same_sender'),
        //         ]);
        //         return 'false';
        // }
        if ($request->ajax()) {
            if ($request->user()->is_expired_subscription) {
                session()->flash('alert', [
                    'type' => 'danger',
                    'msg' =>
                        __('system.subscription_expired'),
                ]);
                return 'false';
            }
            ...
            if (
                ...
                session()->flash('alert', [
                    'type' => 'danger',
                    'msg' => __('validation.campaign_recipients_not_match'),
                ]);
                return 'false';
            }
            ...
            switch ($request->type_message) {
                ...
                case 'image':
                    ...
                    if (!in_array($ext, $allowext)) {
                        return response()->json([
                            'status' => 'error',
                            'msg' => __('validation.allowed_type_file'),
                        ]);
                    }
                    $msg = [
                ...
                case 'button':
                    if ($request->image) {
                        ...
                        if (!in_array($ext, $allowext)) {
                            session()->flash('alert', [
                                'type' => 'danger',
                                'msg' => __('validation.allowed_type_image'),
                            ]);
                            return false;
                        }
                    }
                ...
                case 'template':
                    try {
                        ...
                        session()->flash('alert', [
                            'type' => 'danger',
                            'msg' => __('validation.error_occured'),
                        ]);
                        return true;
                    }
                ...
                case 'list':
                    if (!$request->list1) {
                        session()->flash('alert', [
                            'type' => 'danger',
                            'msg' => __('validation.campaign_req_list_select'),
                        ]);
                        return 'false';
                    }
            ...
            // insert many to blast table, with receiver = in destination
                ...
                session()->flash('alert', [
                    'type' => 'success',
                    'msg' => __('validation.blast_message_scheduled_success'),
                ]);
                return true;
    ...
    public function sendBlast($data, $delay, $campaign)
    {
        try {
            ...
            session()->flash('alert', [
                'type' => 'danger',
                'msg' => __('validation.error_server_node'),
            ]);
            return false;
        }
    }
    ...
    public function makeTemplateButton($templateButton, $no)
    {
        ...
        if (!in_array($type, $allowType)) {
            return redirect(route('autoreply'))->with('alert', [
                'type' => 'danger',
                'msg' => __('validation.campaign_req_template_not_valid'),
            ]);
        }
