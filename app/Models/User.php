...
class User extends Authenticatable
{
    ...
    public function getExpiredSubscriptionAttribute(){
        if($this->active_subscription == 'inactive'){
            return __('system.subscription_inactive');
        } else if($this->active_subscription == 'lifetime'){
            return __('system.subscription_lifetime');
            ...
            if($expired_date < $current_date){
                return __('system.subscription_expired');
            } else {
                // count days
                $days = $expired_date - $current_date;
                $days = $days / (60 * 60 * 24);
                $days = round($days);
                return __('system.subscription_expire').$days.__('system.subscription_expire_');
            }
        }  
    }
    ...
    public function getTotalDeviceAttribute(){
       $connectedDevice = Number::whereUserId($this->id)->whereStatus('Connected')->count();
         ...
         return __('system.connected').': '.$connectedDevice.', '.__('system.disconnected').': '.$disconnectedDevice;
    }
}
