<x-layout-dashboard title="{{__('system.contacts')}}">
...
@endif
                ...
                <div class="card-header d-flex justify-content-between">
                    <form action="{{route('deleteAll')}}" method="POST">
                        ...
                        <button type="submit" name="deleteAll" class="btn btn-danger "><i class="material-icons-outlined">contacts</i>{{__('system.button_delete_all')}}</button>
                   </form>
                   {{--   <button type="button" class="btn btn-primary " data-bs-toggle="modal" data-bs-target="#selectNomor"><i class="material-icons-outlined">contacts</i>Generate Kontak</button> --}}
                    <div class="d-flex justify-content-right">
                        <form action="{{route('exportContact')}}" method="POST">
                            @csrf
                            <input type="hidden" name="tag" value="{{$tag->id}}">
                            <button type="submit" name="" class="btn btn-warning "><i class="material-icons">download</i>{{__('system.button_export')}} (xlsx)</button>
                        </form>
                        <button type="button" class="btn btn-primary mx-2" data-bs-toggle="modal" data-bs-target="#importContacts"><i class="material-icons-outlined">upload</i>{{__('system.button_import')}} (xlsx)</button>
                        <button type="button" class="btn btn-primary " data-bs-toggle="modal" data-bs-target="#addNumber"><i class="material-icons-outlined">add</i>{{__('system.button_add')}}</button>
                    </div>
                </div>
                <div class="row">
                    <div class="col">
                        <div class="card">
                            <div class="card-header d-flex justify-content-between">
                                <h5 class="card-title">{{__('system.contacts_lists_from')}}<span class="badge badge-primary">{{$tag->name}}</span></h5>
                                               ...
                                               <div class="d-flex justify-content-center">
                                                   ...
                                                   <form action="{{route('contactDeleteOne',$contact->id)}}" method="POST">
                                                    ...
                                                    @csrf
                                                       ...
                                                       <button type="submit" name="delete" class="btn btn-danger btn-sm"><i class="material-icons">delete_outline</i>{{__('system.button_delete')}}</button>
                                                    </form>
                                               </div>
                                            </td>
                                        </tr>
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="exampleModalLabel">{{__('system.button_add')}} {{__('system.contact')}}</h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <div class="modal-body">
                    <form action="{{route('addcontact')}}" method="POST" enctype="multipart/form-data">
                        @csrf
                        <label for="name" class="form-label">{{__('system.table_name')}}</label>
                        <input type="text" name="name" class="form-control" id="name" required>
                        <label for="number" class="form-label">{{__('system.table_number')}}</label>
    ...
    <div class="modal fade" id="importContacts" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="exampleModalLabel">{{__('system.button_import')}} {{__('system.contacts')}}</h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <div class="modal-body">
                    <form action="{{route('importContacts')}}" method="POST" enctype="multipart/form-data">
                        @csrf
                        <label for="fileContacts" class="form-label">{{__('system.table_file')}}</label>
                ...
                <div class="modal-footer">
                    <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">{{__('system.button_cancel')}}</button>
                    <button type="submit" name="submit" class="btn btn-primary">{{__('system.button_import')}}</button>
                    </form>
                </div>
            </div>
        </div>
